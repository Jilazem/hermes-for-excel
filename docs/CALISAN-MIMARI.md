...
```
┌─ Win11 (Excel istemcisi) ───────────────────┐        ┌─ GX10 (<GX10-IP>, Linux) ─────────┐
...
**Neden proxy?** Office add-in WebView2'si bir **LAN IP'sine (<GX10-IP>)
...
1. **GX10:** `bridge/server.mjs` çalışır (systemd), `.env` → `HERMES_HOST=0.0.0.0`,
   `HERMES_PUBLIC_HOST=<GX10-IP>`, `HERMES_TLS_CERT/KEY=certs/server.*`,
   `HERMES_PROVIDER=gateway`, `HERMES_GATEWAY_URL=http://127.0.0.1:8646/...`.
   Sertifika `install/configure-remote.mjs --host <GX10-IP>` ile (≤398 gün).
2. **Win11 — bir kez:**
...
WebView2 sandbox'ı `<GX10-IP>` gibi yerel-ağ IP'lerine erişemez (public
